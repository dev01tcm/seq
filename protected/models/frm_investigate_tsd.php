...
class frm_investigate_tsd extends CFormModel	  
{
	public $id;
	public $doc_no;
	public $doc_date;	
	...
	public function save_insert()
	{
		...
		*/
		...
		$sql = "insert into tran_file_tsd (object_group,object_type,create_date,create_by) ";
		...
		if($command->execute()) {		
			...
			Yii::app()->session['file_id_std'] = $file_id;
			
			$sql = "INSERT INTO tran_exportreq_tsd (code,doc_no,doc_date,con_st,con_en,min_id,max_id,cnt,file_id,create_type,create_date,create_by) ";
			...
			$command->bindValue(":con_st", $this->con_st_tsd);
			$command->bindValue(":con_en", $this->con_en_tsd);
			$command->bindValue(":cntmin", $this->cntmin_tsd);	
			$command->bindValue(":cntmax", $this->cntmax_tsd);	
			$command->bindValue(":cnt", $this->cnt_tsd);	
			...
			if($command->execute()) {
				...
				$sql = "update tran_request_tsd set code=:code, status=2, update_date=now(), update_by=$createby ";
				$sql.= "where status=1 and (id>='".$this->cntmin_tsd."' and id<='".$this->cntmax_tsd."') ";
				$command=yii::app()->db->createCommand($sql);
				$command->bindValue(":code", $code_new);				
				if($command->execute()) {
					
					//$ids = Yii::app()->db->getLastInsertID();
					$sql = "insert into tran_exportreq_item_tsd (exportreq_id,code,doc_no,doc_date,acc_employer,business_type, ";
					...
					$sql.= "from tran_request_tsd ";			
					$sql.= "where status=2 and (id>='".$this->cntmin_tsd."' and id<='".$this->cntmax_tsd."') and code='".$code_new."' ";
					$command=yii::app()->db->createCommand($sql);
					$command->bindValue(":code", $code_new);
					$command->bindValue(":doc_no", $this->doc_no);
					$command->bindValue(":doc_date", $this->doc_date);
