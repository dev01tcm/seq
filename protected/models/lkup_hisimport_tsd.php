...
class lkup_hisimport_tsd extends CActiveRecord
{
...
*/
		$sql ="SELECT code as id , code ,doc_no,DATE_FORMAT(DATE_ADD(doc_date, INTERVAL 543 YEAR),'%d/%m/%Y   %T') as doc_date from tran_exportreq_tsd a inner join tran_securities_summary b on a.code = b.request_id ";
...
public function getdetail_tsd($code=null)
		{
		
			$sql ="SELECT total_qty,not_fount_qty,book_closing_qty,registration_qty,request_id ,DATE_FORMAT(DATE_ADD(import_datetime, INTERVAL 543 YEAR),'%d/%m/%Y   %T') as import_datetime from tran_securities_summary where request_id='".$code."'";
			$data1 =Yii::app()->db->createCommand($sql)->queryAll();
			
			return $data1;
		}
	
public function getsreachdetail_tsd($code=null)
	{
	...
	public function getsreachdetail_tsd_book($code=null)
	{
	...
	public function getdatadetail_tsd($code=null)
	{
		
		$sql ="SELECT DATE_FORMAT(DATE_ADD(import_datetime, INTERVAL 543 YEAR),'%d/%m/%Y') as import_datetime, a.code ,a.doc_no,DATE_FORMAT(DATE_ADD(doc_date, INTERVAL 543 YEAR),'%d/%m/%Y') as doc_date from tran_exportreq_tsd a inner join tran_securities_summary b on a.code = b.request_id  where a.code='".$code."' ";
		$data1 =Yii::app()->db->createCommand($sql)->queryAll();
		return $data1;
	}
