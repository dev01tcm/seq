...
class lkup_investigate_tsd extends CActiveRecord
{
    ...
    //$sheet->getStyle("A1:B1")->applyFromArray($style);
		...
		//Yii::app()->CommonFnc->log_events("export", "step6_1", "auo job");		
			...
			$txtfileName = $dr_folder.Yii::app()->params['prg_ctrl']['path']['closepath']."SSO_".$code."_TSD.txt";
			$x = 2;	
		...
		//Yii::app()->CommonFnc->log_events("export", "step6_2", "auo job");				
				...
				$excelFileName = $dr_folder.Yii::app()->params['prg_ctrl']['path']['closepath']."SSO_".$code."_TSD.xls";
			//	$objWriter->save($excelFileName);	
				...
				$filezip = Yii::app()->params['prg_ctrl']['path']['media'].Yii::app()->params['prg_ctrl']['path']['closepath']."SSO_".$code."_TSD.zip";
				$filename = "SSO_".$code."_TSD.zip";
		...
		$sql = "update tran_file_tsd set name=:name, upload_path=:path, upload_url=:url,file_type=:filetype,file_size=:file_size,object_id=:object_id ";			
	...
	public function datein($doc_no=null,$st_date=null,$en_date=null) {		
		...
		$sql ="select count(*) as number from tran_request_tsd where status!=0 and status=2 ";
