...
class lkup_request extends CActiveRecord
{
	...
	public function search($keyword=null,$acc_emp=null,$iden=null,$status1=null,$status2=null,$status3=null,$status4=null) {
		...
		$sql= "select a.id,a.code,c.doc_no as docno_exp,a.doc_no as doc_no,a.doc_date, ";
	...
	public function searchdetail($doc_no=null,$doc_date=null,$doc_no_tsd=null) {
		...
		if($doc_no!='' && $doc_date!=''){							
			$sqlCon.= " and a.doc_no='".$doc_no."' and a.doc_date='".$doc_date."'  ";				
		}
		if($doc_no_tsd!='' && $doc_date!=''){							
			$sqlCon1.= " and a.doc_no='".$doc_no_tsd."' and a.doc_date='".$doc_date."'  ";				
		}
			
		$count=Yii::app()->db->createCommand("select count(*) as aa from tran_request a where 1=1 and dep_id='".$dep_id."' ".$sqlCon." and status!=0 union select count(*) as aa from tran_request_tsd a where 1=1 and dep_id='".$dep_id."' ".$sqlCon1." and status!=0")->queryScalar();
		
		$sql="select a.id,a.doc_no,a.acc_employer, ";	
		$sql.="CONCAT(ifnull(date_format(a.birth,'%d/%m/'),''),ifnull(date_format(a.birth,'%Y')+543,'') )as birth, ";
		...
		$sql.="CONCAT(ifnull(a.pid,''),' ',ifnull(a.cid,'') )as pid  , ";
		$sql.="CONCAT(a.name,' ', a.lname, ' ', ifnull(a.company_name,'') ) full_name,'Bank' as type ";	
		$sql.="from tran_request a ";
		...
		$sql.="CONCAT(ifnull(date_format(a.doc_date,'%d/%m/'),''),ifnull(date_format(a.doc_date,'%Y')+543,'') )as doc_date, ";
		$sql.="CONCAT(ifnull(a.pid,''),' ',ifnull(a.cid,'') )as pid  , ";
		$sql.="CONCAT(a.name,' ', a.lname, ' ', ifnull(a.company_name,'') ) full_name , 'TSD' as type " ;	
		$sql.="from tran_request_tsd a ";
		$sql.="where 1=1 ".$sqlCon1." and status!=0";
		//$sql.="from tran_request_item a ";
		//echo var_dump($sql);
		//exit;
		return new CSqlDataProvider($sql, array(
			...
			'sort'=>array(
				'attributes'=>array(
					 'id', 'doc_no', 'doc_date', 'acc_employer', 'full_name', 'pid','business_name','birth','address','create_date'
				),
			),
			'pagination'=>array(
				'pageSize'=>Yii::app()->params['prg_ctrl']['pagination']['default']['pagesize'],
	...
	public function getRequestfrom($doc_no = null,$doc_date = null)
	{
	   ...
	   $sql.= "CONCAT(ifnull(date_format(doc_date,'%d/%m/'),''),ifnull(date_format(doc_date,'%Y')+543,'') )as doc_date ";
	   $sql.= "from tran_request where status!=0 and doc_no='".$doc_no."' and doc_date='".$doc_date."' group by doc_no ";	   
	   $rows =Yii::app()->db->createCommand($sql)->queryAll();
	   return $rows;
	}
