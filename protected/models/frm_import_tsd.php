...
class frm_import_tsd extends CFormModel
{	
	public $name;
	public $size;	
	public $path;
...
//public function save_insert($name=null, $fullpath=null, $fullurl=null, $size=null, $type=null)
  ...
  public function importtsd($fileload)
		{
			
			
			$i=0;
			$a=0;
			$xml=simplexml_load_file("media/$fileload");
			...
			/*
			$sql = "insert into tran_securities_summary (request_id,import_datetime) ";
			$sql.= "VALUES(:request_id,now())";
			$command=yii::app()->db->createCommand($sql);
			$command->bindValue(":request_id", $RequestID);
			if($command->execute()) {
			...
			foreach($data as $key => $val)
				{
					...
					//	exit;
						...
						foreach($val as $key2 => $val2)
						{
							...
							$reference_id=$val[$a]['ORIGReferenceID'];
						//	var_dump($reference_id);
					
							
								$AccountID=$val[$a]['CustomerInfo']['AccountID'];
								
								$val3=$val2['CustomerInfo']['SecuritiesInfo'];
								foreach($val3 as $key3 => $val4)
									{
										
										$SecuritiesAbbr=$val4['SecuritiesAbbr'];
										$SecuritiesNameThai=$val4['SecuritiesNameThai'];
										$SecuritiesType=$val4['SecuritiesType'];
										$EffectiveDate=$val4['EffectiveDate'];
										$CertificateNo=$val4['CertificateNo'];
										$SecuritiesShares=$val4['SecuritiesShares'];
										$CertStatus=$val4['CertStatus'];
														
														$sql = "insert into tran_securities_registration(request_code,account_id,reference_id,securities_abbr,securities_namethai,securities_type,effective_date,certificate_no,securities_shares,cert_status) ";
														$sql.= "VALUES(:request_code,:account_id,:reference_id,:SecuritiesAbbr,:SecuritiesNameThai,:SecuritiesType,:EffectiveDate,:CertificateNo,:SecuritiesShares,:CertStatus)";
														$command=yii::app()->db->createCommand($sql);
														$command->bindValue(":request_code", $RequestID);
														$command->bindValue(":account_id", $AccountID);
														$command->bindValue(":reference_id", $reference_id);
														$command->bindValue(":SecuritiesAbbr", $SecuritiesAbbr);
														$command->bindValue(":SecuritiesNameThai", $SecuritiesNameThai);
														$command->bindValue(":SecuritiesType", $SecuritiesType);
														$command->bindValue(":EffectiveDate", $EffectiveDate);
														$command->bindValue(":CertificateNo", $CertificateNo);
														$command->bindValue(":SecuritiesShares", $SecuritiesShares);
														$command->bindValue(":CertStatus", $CertStatus);
														if($command->execute()) {
		...
		//	exit;
			...
			$sql = "insert into tran_securities_summary (request_id,import_datetime,registration_qty) ";
			$sql.= "VALUES(:request_id,now(),:TotalAccount)";
			$command=yii::app()->db->createCommand($sql);
			$command->bindValue(":request_id", $RequestID);
			$command->bindValue(":TotalAccount", $TotalAccount);
			if($command->execute()) {
...
public function importtsd_book_closing($fileload)
		{
			...
			foreach($data as $key => $val)
				{
					...
					//	exit;
						...
						//	var_dump($reference_id);
								...
								$AccountID=$val[$a]['CustomerInfo']['AccountID'];
								$ResultFlag=$val[$a]['CustomerInfo']['ResultFlag'];
								$val3=$val2['CustomerInfo']['SecuritiesInfo'];
								foreach($val3 as $key3 => $val4)
									{
										$AuditType=$val4['AuditType'];
										$SecuritiesAbbr=$val4['SecuritiesAbbr'];
										$SecuritiesNameThai=$val4['SecuritiesNameThai'];
										$SecuritiesType=$val4['SecuritiesType'];
										$BookClosingDate=$val4['BookClosingDate'];
										$EffectiveDate=$val4['EffectiveDate'];
										$SecuritiesShares=$val4['SecuritiesShares'];
										$PartiNameThai=$val4['PartiNameThai'];
										$PartiNo=$val4['PartiNo'];
														
														$sql = "insert into tran_securities_book_closing(request_code,account_id,reference_id,result_flag,audit_type,securities_abbr,securities_namethai,securities_type,book_closing_date,effective_date,securities_shares,parti_no,parti_name_thai) ";
														$sql.= "VALUES(:request_code,:account_id,:reference_id,:ResultFlag,:AuditType,:SecuritiesAbbr,:SecuritiesNameThai,:SecuritiesType,:BookClosingDate,:EffectiveDate,:SecuritiesShares,:PartiNo,:PartiNameThai)";
														$command=yii::app()->db->createCommand($sql);
														$command->bindValue(":request_code", $RequestID);
														$command->bindValue(":account_id", $AccountID);
														$command->bindValue(":reference_id", $reference_id);
														$command->bindValue(":ResultFlag", $ResultFlag);
														$command->bindValue(":AuditType", $AuditType);
														$command->bindValue(":SecuritiesAbbr", $SecuritiesAbbr);
														$command->bindValue(":SecuritiesNameThai", $SecuritiesNameThai);
														$command->bindValue(":SecuritiesType", $SecuritiesType);
														$command->bindValue(":BookClosingDate", $BookClosingDate);
														$command->bindValue(":EffectiveDate", $EffectiveDate);
													//	$command->bindValue(":CertificateNo", $CertificateNo);
														$command->bindValue(":SecuritiesShares", $SecuritiesShares);
														$command->bindValue(":PartiNo", $PartiNo);
														$command->bindValue(":PartiNameThai", $PartiNameThai);
														if($command->execute()) {
		...
		//	exit;
			...
			$sql = "update tran_securities_summary set book_closing_qty='".$TotalAccount."' where request_id='".$RequestID."' ";					
			$command=yii::app()->db->createCommand($sql);
			if($command->execute()) {
...
public function importtsd_not_found($fileload)
	{
			...
			foreach($data as $key => $val)
				{
					...
					//	exit;
						...
						//	var_dump($reference_id);
							...
							$sql = "insert into tran_securities_not_found (request_id,reference_id) ";
							$sql.= "VALUES(:request_id,:reference_id)";
							$command=yii::app()->db->createCommand($sql);
							$command->bindValue(":request_id", $RequestID);
							$command->bindValue(":reference_id", $reference_id);
							if($command->execute()) {
		...
		/*	
			$sql = "update tran_request_tsd set status=4 , update_by=$updateby where code='".$RequestID."' and pid='".$reference_id."' ";					
			$command=yii::app()->db->createCommand($sql);
			if($command->execute()) {
		...
		*/			
		...
		$sql = "update tran_securities_summary set not_fount_qty='".$TotalAccount."' where request_id='".$RequestID."' ";					
		$command=yii::app()->db->createCommand($sql);
		if($command->execute()) {
...
public function TotalAccount_TSD($total)
	{
		$updateby = !Yii::app()->user->isGuest?Yii::app()->user->id:0;	
		$RequestID=Yii::app()->session['RequestID'];
		
		$sql = "update tran_securities_summary set total_qty='".$total."' where request_id='".$RequestID."' ";					
		$command=yii::app()->db->createCommand($sql);
		if($command->execute()) {
		...
		$sql = "update tran_request_tsd set status=4 , update_by=$updateby where code='".$RequestID."'";					
		$command=yii::app()->db->createCommand($sql);
		if($command->execute()) {
