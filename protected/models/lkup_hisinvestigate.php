...
class lkup_hisinvestigate extends CActiveRecord
{
	...
	public function search($keyword=null) {
		...
		$count=Yii::app()->db->createCommand('select count(*) from tran_exportreq a where a.status!=0 ')->queryScalar();		
		...
		$sql.= " left join tran_exportreq_item b on b.exportreq_id=a.id ";
	...
	public function searchform($id = null,$keyword=null)
	{
		...
		$count=Yii::app()->db->createCommand("select count(*) from tran_exportreq_item a where a.status!=0 and a.exportreq_id='".$id."' ".$sqlCon)->queryScalar();
		...
		$sql.="left join tran_exportreq b on b.id = a.exportreq_id ";
		$sql.="left join tran_file c on c.id=b.file_id ";
