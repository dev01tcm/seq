...
class frm_hisinvestigate extends CFormModel
{
	 public $id;
	 public $code;
	 public $doc_no;
	...
	public function rules()
 	{
  		return array(
   			array('code', 'id', 'doc_no', 'doc_date', 'safe'),    
  		);
 	}
	...
	public function file_investigate()
 	{
		...
		date_default_timezone_set("Asia/Bangkok");
  		...
  		$sql = "update tran_exportreq set doc_no=:doc_no, doc_date=:doc_date, update_date=now(), update_by=$updateby where id='".$this->id."'";
  		$command=yii::app()->db->createCommand($sql);   
		$command->bindValue(":doc_no", $this->doc_no);  
		$command->bindValue(":doc_date", $this->doc_date);   
   		if($command->execute()) {
    		$sql = "update tran_exportreq_item set doc_no=:doc_no, doc_date=:doc_date, update_date=now(), update_by=$updateby where exportreq_id='".$this->id."'";
    		$command=yii::app()->db->createCommand($sql);   
    		$command->bindValue(":doc_no", $this->doc_no);  
    		$command->bindValue(":doc_date", $this->doc_date);   
     		if($command->execute()) { 
