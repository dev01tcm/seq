<?php
	...
	class ConductPDF extends CustomTCPDF {	
		...
		function Header(){
        	...
        	$this->Cell(0, -7,"ผลการตรวจสอบบัญชีเงินฝาก", 0, 1, 'C');
			$this->Ln(7);
			$this->SetFont('thsarabun', 'b', 14);
			$this->Cell(90,-10,'ลำดับที่ : '.$this->para01.'                                      '.'ชุดหนังสือ :  ' .$this->para02.'                          '.'วันที่หนังสือ(ส่งออก) : '.$this->para03,0,0,"L");
			$this->Ln(7);			
			$this->SetFont('thsarabun', 'b', 14);
			$this->Cell(90,-10,'เลขที่บัญชีนายจ้าง : '.$this->para04.'          คำนำหน้าชื่อ/ประเภทธุรกิจ : '.$this->para07.'           ชื่อ - สกุล : '.$this->para05.'             '.'เลขป.ช.ช / เลขทะเบียนพาณิชย์ : '.$this->para06,0,0,"L");				
			$this->Ln(7);
	...
	// set margins
	$pdf->SetMargins(6, 45 , 10);//$pdf->SetMargins(PDF_MARGIN_LEFT, 30.8, PDF_MARGIN_RIGHT);
	...
	$groupdata=lookupdata::getDetail($id);
	...
	foreach ($groupdata as $grouprow)
	{
			...
			$pdf->para01=$grouprow['id'];
			$pdf->para02=$grouprow['code'];
			$pdf->para03=$grouprow['doc_date'];
			$pdf->para04=$grouprow['acc_employer'];
			$pdf->para05=$grouprow['full_name'];
			$pdf->para06=$grouprow['pid'];
			$pdf->para07=$grouprow['business_name'];
			
			$pdf->AddPage('L','A4');
	
			// Get data
			$data=lkup_requestresult::getDetailpdf($id);;
			...
			{					
				...
				$h.='<td style="width:142px;text-align:left;">'.$row['bank_name'].'</td>';	
				$h.='<td style="width:50px;text-align:center;">'.$row['bank_id'].'</td>';			
